...
export default function Home() {
  ...
  const input =
    "w-full rounded-[2px] border border-zinc-700 bg-zinc-900 px-4 py-3 text-lg outline-none focus:border-gold";
  const button =
    "w-full rounded-[2px] px-4 py-3 text-lg font-semibold disabled:opacity-50";

  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center gap-10 px-4 py-10">
      <header>
        <h1 className="font-cond text-[40px] leading-none font-extrabold uppercase">easyATC</h1>
        <p className="mt-1 text-zinc-400">Seguimiento en vivo del ejercicio de control.</p>
      </header>

      <form onSubmit={join} className="space-y-3">
        <h2 className="kicker text-gold">Entrar con código</h2>
        <input
          ...
          autoComplete="off"
        />
        <button className={`${button} bg-gold text-zinc-950`} disabled={busy}>
          Entrar
        </button>
      </form>

      <form onSubmit={create} className="space-y-3 border-t border-zinc-800 pt-8">
        <h2 className="kicker text-gold">Crear sesión nueva</h2>
        <input
          className={input}
      ...
      {error && <p className="text-center text-ko">{error}</p>}
    </main>
  );
}
